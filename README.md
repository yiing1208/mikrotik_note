...
add address=(此台設備的IP,e.g. 192.168.1.1) interface=bridge1 network=(子網路遮罩,e.g. 192.168.1.0)
```
###如果是撥號(hinet)
```
...
```
##防止WinBox(port 8291)被try(允許錯3次,3次全錯阻擋10min)(ref:https://mhelp.pro/mikrotik-fail2ban-blocking-brute-force-attacks/)
```
/ip firewall filter
...
```
##Wireguard (對端內網:192.168.10.0/24,對端tunnel ip:10.255.255.1/24)
```
/interface wireguard
